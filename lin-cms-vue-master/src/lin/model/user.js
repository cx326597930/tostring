...
export default class User {
  ...
  static async getToken(username, password) {
    let tokens = await post('cms/user/login', {
      username,
      password,
    })
  ...
  static async getInformation() {
    ...
    console.log('接口getInformation()返回的用户信息:', info)
    const avatar = info.avatar.replace('http://localhost:5000/', process.env.VUE_APP_BASE_URL)
    info.avatar = avatar
    console.log('处理完成接口getInformation()返回的用户信息:', info)
